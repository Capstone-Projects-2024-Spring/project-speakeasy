            ...
            if (matches) {
              return matches.map((match) => {
                ...
                if (questionMatch && optionsMatch) {
                  ...
                  let options = optionsMatch.map((option) => option.trim());
                    // Ensure at least 3 non-empty options and filter out questions with "Option N/A"
                    options = options.filter(Boolean); // Remove empty options
                    if (options.length < 3) {
                        return null; // Discard question with less than 3 valid options
                    }    
                    // Identify and remove the correct answer from options (modified logic)
                    const correctAnswerMatch = message.text.match(/([A-Z]\.)(.*?)(?=\s*[A-Z]\.|$)/g);
                    if (correctAnswerMatch) {
                        const correctAnswerText = correctAnswerMatch[0].replace(/[A-Z]\.\s*/, '');
                        options = options.filter((opt, index) => opt.toUpperCase() !== correctAnswerText.toUpperCase() || index === 0); // Keep the first occurrence (if present) for potential answer shuffling by the bot
                    }
